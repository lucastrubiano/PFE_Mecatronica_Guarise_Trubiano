...
from config import EYES_LMS_NUMS
from config import INNER_LIP_LMS_NUMS
from config import LEFT_IRIS_NUM
from config import OUTTER_LIP_LMS_NUMS
from config import RIGHT_IRIS_NUM


class FacialFeaturesExtractionSystem:
    """
    Class FacialFeaturesExtractionSystem is the class
    that runs the facial features extraction system.
    """

    def __init__(self, head_pose) -> None:
        """
        Initialize Facial features extraction
        system which use mediapipe library
        """
        ...
        self.Head_pose = HeadPoseEstimator(show_axis=self.head_pose)
    ...
    def pre_processing(self, frame: np.array) -> (np.array, tuple):
        ...
        processed_frame = np.expand_dims(
            cv2.bilateralFilter(processed_frame, 5, 10, 10), axis=2,
        )
        processed_frame = np.concatenate(
            [processed_frame, processed_frame, processed_frame], axis=2,
        )
    ...
    def face_detection(self, frame: np.array) -> np.array:
        ...
        multiple_face_landmakrs = self.detector.process(
            frame,
