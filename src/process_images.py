import cv2
from os import environ as env, path, makedirs, listdir, rename
...
class ProcessImages:
    ...
    IMG_POSITIVE_DIR = env["IMG_POSITIVE_DIR"].format(width=WIDTH, heigth=HEIGHT)
    IMG_NEGATIVE_DIR = env["IMG_NEGATIVE_DIR"].format(width=WIDTH, heigth=HEIGHT)
    ...
    def __process_images(self, input_dir, output_dir):
        ...
        for i, file in enumerate(files):
            rename(
                path.join(input_dir, file), path.join(input_dir, "{:08}.jpg".format(i))
            )
